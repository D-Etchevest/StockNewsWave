...
# Transformations ####
All2 <- rbind(GoogleBars[[1]],GoogleBars[[2]],FacebookBars[[1]][-7],FacebookBars[[2]][-7],
...
for (p in names(table(All$Instrument))){
  data <- filter(All, Instrument == p & Event == "Yes" & index %in% indexDiff_Tick$index)
  for(f in names(table(All$Bar))){
...
for (j in names(table(All$Instrument))){
  for(f in names(table(All$Bar))){
  data <- filter(All, Instrument == j &  Bar == f)
...
```{r}#
low <- All %>% mutate(date_col = date(timestamp)) %>% 
